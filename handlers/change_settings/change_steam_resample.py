...
from data.messages import (
    ...
    SUCCESSFULLY_SAVE_STEAM_RESAMPLE_THRESHOLD_MESSAGE,
    ERROR_SAVE_STEAM_RESAMPLE_THRESHOLD_MESSAGE,
...
@dp.callback_query_handler(
    lambda c: c.data == CHANGE_STEAM_RESAMPLE_DATA,
    state=ChangeSettingsStatesGroup.change_settings
)
async def change_steam_resample(callback: types.CallbackQuery, state: FSMContext) -> None:
    user_id = callback.from_user.id

    await keyboards_clear(user_id=user_id, state=state)
    await bot.send_message(chat_id=user_id, text=CHANGE_STEAM_RESAMPLE_THRESHOLD_MESSAGE)

    await ChangeSettingsStatesGroup.change_steam_resample.set()


@dp.message_handler(
    content_types=types.ContentTypes.TEXT,
    state=ChangeSettingsStatesGroup.change_steam_resample
)
async def enter_steam_resample(message: types.Message, state: FSMContext) -> None:
